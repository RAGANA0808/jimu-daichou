...
- **PR #5**（docs 反映）/ **PR #6**（`feat/nenki-next-countdown`: 年忌バッジに「次の法要まであと◯年」）→ **MERGED**。
- 現在 **main = `3d2d12f`**（PR #6 反映）。マージ済みブランチは local/remote とも削除済み・tree クリーン。
...
**いつでも入れてよい改善（安価・UI層中心）**: ~~年忌バッジに「次回まであと◯年」明示~~ ✅(PR #6) / タイル面に種別+番号+使用者姓 / 一覧の戒名順ソート・並べ替え導線 / 詳細ページのクエリ集約（=問題A選択肢3・PR #4 で緩和済・根治は将来）。
